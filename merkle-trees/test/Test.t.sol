...
contract MockImplementation is IMockImplementation {
    ...
    function revertingFunction() public pure override {
...
contract ProxyTest is Test {
    ...
    function testImplementationAddressIntegrity() public {
        ...
        // Test setting zero address (should revert and not change the implementation)
        vm.expectRevert("Proxy: new implementation is the zero address");
    ...
    function testProxyFunctionality() public {
        ...
        IMockImplementation(address(proxy)).setValue(42);
        assertEq(IMockImplementation(address(proxy)).value(), 42);

        // Test reverting calls
        vm.expectRevert("This function always reverts");
        IMockImplementation(address(proxy)).revertingFunction();
    }

    function testUpgradeAtomicity() public {
        // Prepare calldata for setValue(100)
        bytes memory data = abi.encodeWithSelector(IMockImplementation.setValue.selector, 100);
        ...
        assertEq(proxy.implementation(), address(implementation));
        assertEq(IMockImplementation(address(proxy)).value(), 100);
    }
    ...
    function testGasLimitHandling() public {
        ...
        (bool success,) =
            address(proxy).call{gas: 1000}(abi.encodeWithSelector(IMockImplementation.setValue.selector, 42));
        assertFalse(success);
    }
